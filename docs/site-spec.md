...
- **サイト審査: 2026-07-10申請 → 2026-07-17不合格（「有用性の低いコンテンツ」）→ 対策実施中**。
  ...
  **未対応の残項目（2026-07-13全ページ監査で特定。再申請はこれらの完了後）**:
  (1)一覧4ページ（blog.html・blog/・mahjong/・news/）が静的HTML本文ゼロのJSシェルのまま広告枠+インデックス対象 → 記事リストの静的HTML化+カテゴリ説明文
  (2)about.htmlへの静的リンクが全サイトに0本（トップのフッターにも無い）→ 全フッターに「運営者について」追加
  (3)フッターのリンク構成がページ群で5パターンに分裂 → 1セットに統一
  (4)1,000字未満ページ: faq(932字)・score-basics(922字)は増補、news2本はnoindex+sitemap除外
  (5)全記事が同一テンプレ（同じ装飾SVG・同じCTA・公開日集中）で量産に見える → 記事ごとに固有の図版/体験/データを注入
  (6)ads.jsが非表示ビュー内の広告枠も初期化（共有URL起動時にコンテンツなし画面で広告リクエスト）→ 可視ビューのみ初期化
  (7)blog.htmlとblog/がsitemapに重複 → canonical一本化。

...
- **2026-09-26: 見るだけのリンク（sessionViews / views）を追加。コンソールへの再適用が必要**
  （未適用の間は共有シートに「見るだけのリンク」の欄が出ないだけ。点数入力・共有・仲間ページは影響なし）
  - sessionViews/$sid: IDを知っていれば読める。書けるのは「まだ無くて、その試合がある」ときだけ（作り直し・上書き不可）。試合が消えたら消せる
  - views/$vid: 読めるのは s（写し）だけ。書き込みは p＝「試合ID_n」・n を毎回変えることが条件で、p から取り出した試合IDの試合が存在し、
    前回と同じ試合IDであること（リンクだけ知っている人・別の試合の人は書けない）。削除は元の試合が消えたあとだけ。s・p・n 以外のキーは不可
  - 確認: `python tools/view_rules_test.py`（本番DB・ログイン不要・20項目）
- **2026-09-26: 試合の記録の確定（sessions/$id の lockAt・endedAt）を追加。コンソールへの再適用が必要**。
  **アプリ（lockAt を書く版）より先に貼ること**（先にアプリを出すと、未知のキーとして保存が全部拒否される）
  - `.write` に「lockAt が無い、またはサーバー時刻 now が lockAt より前」を足した＝**締め切りを過ぎた試合は誰も書き換えられない・消せない**（読むのはできる）
  - lockAt: 数値。変えるときは「今から11〜25時間後」の範囲だけ（即確定させるいたずら・いつまでも確定しない抜け道を防ぐ）。一度付いたら消せない
  - endedAt（対局を終了した時刻）: 数値。変えるときは「今±10分」だけ。endedAt があるときは lockAt ≦ endedAt＋13時間（終了後に締め切りを延ばせない）。
    endedAt を消す（再開）のは締め切り前なら誰でもできる
  - 確認: `python tools/lock_rules_test.py`（本番DB・ログイン不要・14項目）。「締め切り後は書けない」は即確定を禁じているのでその場では試せない →
    `--canary` で確認用の試合を置き、12時間後に `--check-canary`（確定した確認用の試合は消せないので名前「ロック確認用」のまま残る）
...
- **サイト審査: 2026-07-10申請 → 2026-07-17不合格 → 対策後に再申請 → 2026-08-25に再び「有用性の低いコンテンツ」**。
  ...
  **Phase 4（2026-09-25・4回目＝9/20の「有用性の低いコンテンツ」への対応）**: 審査履歴は 7/17・8/3・9/5・9/20 の4回すべて不合格。
  管理画面には「ポリシー違反が見つかりました → 有用性の低いコンテンツ」と出る（ギャンブルの指摘ではない）。今回は「量と体裁」ではなく
  **ツールとしての独自の価値**と**どこにでもある内容の削減**に振った:
  (a) アプリ: 倍率・換算を機能ごと廃止／相手ごとの対戦成績／「あなたはどれ？」／まとめて選ぶ／まとめ（複数ゲームの通算順位）／
      Googleログイン（LOGIN_ENABLED=falseで準備中）／LINEから外のブラウザで開く共有URL
  (b) 審査中は空の広告枠を出さない（ADS_APPROVED=false）
  (c) 記事: 新記事 blog-season-ranking（日をまたいだ通算と相性）。重なる2本（符と翻・牌の種類）を統合して301
...
## ブログ系ページの共通ヘッダー/フッター
- `/assets/blog-site.js` の `renderChrome()` が一元描画（LPと同じ見た目=中央ロゴ48px・右上「マイページ」＝`/#mypage`・緑2列フッター）。ブログはログイン状態を知らないので、文言は「マイページ」で固定（開くとログイン中はマイページ、未ログインはログイン画面）
...
## ギャンブル判定を避けるための線引き（2026-08-26〜）
- AdSense不合格が3回続き、表示は「有用性の低いコンテンツ」のままだったが、
  **賭博（賭け麻雀）を助長するツールと読まれている可能性**を疑って金銭を想起させる面を削った。
  日本では金銭を賭けた麻雀は刑法185条の賭博罪（最高裁昭28.10.7＝金額の多少を問わない）で、
  Googleパブリッシャーポリシーは「違法行為を助長するコンテンツ」を禁止している
- **いちばん危なかったのはトップページの生HTMLに出ていた換算例**:
  「例：スコア倍率が×100の場合、+123.0pts → +12,300bonus」「例：チップ倍率が×500の場合、+5枚 → +2,500bonus」。
  単位を円からbonusに置き換えても、換算式そのものを見せていては意味がない
- 実施した対策（案B）: ①スコア倍率の既定を「なし」に ②倍率をルール設定カードからオプションカードへ移動
  ③換算例を全ページから削除 ④LPの機能カード・イラストから収支/倍率/チップを削除
  ⑤FAQ・お知らせ・記事・規約の表現を「記録」ベースに統一
- **追加対応（2026-08-26）**: 機能は残したまま、画面に出る語を中立化した。
  「収支」→「換算」（スコア換算／チップ換算／換算合計）、表示単位の「bonus」は出さない。
  これで **index.html の生HTMLから「収支」と表示単位の bonus が0件**になった
- **2026-09-25に案C（倍率・換算の機能ごと廃止）を実施**。スコア倍率（×10〜×2000）とチップ倍率（既定×500）は、
  1pts=1,000点なので賭け麻雀のレート表・チップ1枚の相場と同じ数字に見えていた。残したのはチップの枚数の記録だけ
- 規約の「賭博行為との関係について」に、**金銭授受を前提にした一文を置かない**。
  「ユーザー間で金銭のやり取りを行う場合は…法令を遵守の上行ってください」は前段の
  「金銭のやり取りを仲介・助長する機能は含みません」と矛盾し、承知のうえで免責していると読める（2026-08-26に削除）
...
- **会社PC（社内ネットワーク）からの本番確認**: 以前は majasco.jp 自体がZscalerでブロックされていたが（新規登録ドメインカテゴリ）、
  **2026-09-25時点では取得できる**（HTTP 200）。ただしPlaywright・ブラウザペインで本番を開くと**Firebaseにつながらない**ことがある
  （Local Network Access。対処は docs/app-spec.md の落とし穴）。開発検証はlocalhostプレビューが基本で、
  本番で実データの動きを見たいときはユーザーのスマホ回線に依頼する

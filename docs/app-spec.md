...
- **規模の上限**（増えすぎると成り立たなくなるものに置いた線）:
  ...
  ②対局履歴の画像が読める大きさの1枚に収まらなくなる ③rules.jsonのrounds indexが4桁まで。
...
- **表示量の調整（規模が大きいときに画面が長くなりすぎないようにする）**:
  対局履歴は既定で**直近`ROUNDS_PREVIEW`=20試合だけ**を描き、下の「すべての試合を表示する（全n試合）」で全件に切り替える
...
- **まとめ画像**（`openComposeModal`→`runCompose`→`buildComposedCanvas`）: 入れる項目をチェックで選び、
  ...
  **収まらないときの段階的な対処**: ①列が多いほど1列の上限幅を下げる（`COL_MAX`＝27列以上84px／15列以上104px／既定132px。
  40人でも幅8000pxに達しない）②それでも収まらない対局履歴は**薄い点数・チップ差を省いた縮約版**で作り直す
  ...
  **判定は四捨五入後の実ピクセルで行う**（ぴったり上限になる倍率は丸め誤差で1px超えるため、面積由来の倍率は0.999倍しておく）。
  実測: 16人4チーム100試合＝全項目可（対局履歴の文字17px）／40人4チーム100試合＝全項目可（同12px）／
  40人10チーム300試合＝全項目可（同9px）／500試合＝対局履歴の単体のみ案内、他は可

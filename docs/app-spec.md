...
- **引き継ぎURL（第2段階・端末をまたぐ履歴。2026-08）**: 端末ごとに専用ID（**14文字**・セッションIDと同じ文字種）を発行し、
  ...
  - UI: 過去の試合画面のカード`#mylist-card`（`renderMyListCard`。未作成=「引き継ぎURLを作る」／作成済み=URL表示＋コピー。
  ...
  - 復元: `openMyListFromUrl`＝リストを取り込み→無いグループをsessionsから並列取得（新しい順・最大MAX_HISTORY件）→
    ローカルにだけあるグループはサーバー側へ追記（双方向）→**処理後にhashを消す**（再読み込みで二重取り込みしない）

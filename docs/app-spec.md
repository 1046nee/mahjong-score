...
- 総合順位: pts順位がデフォルト。ミニトグル「スコア/チップ」はチップあり🍪のときだけ。
  ...
  **内訳の表（`bonus-detail-table`）も対局履歴・個人成績と同じ体裁**: 先頭セル=「メンバー」／チーム戦はチーム名行あり
...
- **まとめ画像**（`openComposeModal`→`runCompose`→`buildComposedCanvas`）: 入れる項目をチェックで選び、
  ...
  **iOS Safariのcanvas上限（1辺8000px・総ピクセル数16.7M）を超えるとtoBlobが黙ってnullを返し、保存もプレビューも失敗する**。
  ...
  倍率の下限は`IMG_MIN_SCALE`=0.7（本文13pxが約9px＝読める限界）。これでも収まらないときは
  壊れた画像を作らず**`IMG_TOO_LARGE`を返し**、呼び出し側がCSVを案内する。
  **16人4チームの実測: 100試合＝すべて生成可（対局履歴は文字19px）／200試合＝可（同9.6px）／
  400試合＝対局履歴の単体のみ案内メッセージ、他は生成可**

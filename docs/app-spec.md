...
- **引き継ぎURL（第2段階・端末をまたぐ履歴。2026-08）**: 端末ごとに専用ID（**14文字**・セッションIDと同じ文字種）を発行し、
  ...
  - 復元・マージの本体は`mergeMyListData(data)`（openMyListFromUrlとrefreshMyListで共用）＝リストを取り込み→
    無いグループをsessionsから並列取得（新しい順・最大MAX_HISTORY件）→
...
- **参加/観戦タグと「自分の成績シート」（2026-08）**: 会員登録が無くアプリは本人を特定できないため、
  **本人が卓ごとにタグ付けする**設計。分類はlocalStorage `mahjong-tags-v1`＝`{gid:{role:'play'|'watch', me:index}}`
  ...
  - **自分の成績シート**（`view-mystats`。過去の試合の「📊自分の成績シート」ボタンから）:
    ...
    同点は`rankGroups`按分＝個人成績と同じ数え方。行=グループ数/試合数/平均着順/1..4着/トップ率/連対率/ラス回避/
    通算スコア/最高・最低点数＋グループ別内訳（タップで結果詳細）。
    スコアは各グループのルールで計算された値の合算である旨を注記する。
    **表の長文対策**: 項目列は**78px**（「グループ数」「通算スコア」=5文字が収まる幅。64pxでははみ出す）。
...
- **名前の文字数上限**: プレイヤー名=`MAX_NAME_LEN`**16**／チーム名=`MAX_TEAM_LEN`**40**／グループ名=`MAX_GROUP_LEN`**40**。

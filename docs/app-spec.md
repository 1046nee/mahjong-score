...
- **チーム戦のチーム名行**: 対局履歴・その他成績の両表とも、プレイヤー名行の上に`teamNameRowHtml`のチーム名行を置く
  （チームごとにcolspanで1セル・プレイヤー名と同じ文字サイズ・TEAM_HEAD_BG背景）。
  チームの境目は縦区切り線クラス`.tb`（**1px**=本文の横罫線と同じ太さ。見出し=白・本文=薄グレー #e3eaea）を
  チーム名行/名前行/本文の全セルに付けて示す（境目列の算出は`teamBoundarySet`、グループは`teamGroupsOf`）。
  **長いチーム名・プレイヤー名は何度でも折り返す**（`.rounds-table th`にword-break:break-all。列幅を押し広げて表を横長にしない）
- **フィット縮小（.table-fit）**: 対局履歴・その他成績のカード上の表は、画面幅を超えるとき`fitTables()`が
  ラッパーごとzoomで縮小して**左から右まで1画面に収める**（文字も等倍で縮むのではみ出さない）。
  下限`TABLE_FIT_MIN_ZOOM=0.34`≒16人の表がスマホ幅に収まる倍率。それ未満（40人など）は横スクロール併用。
  renderGame/renderDetailBodyの最後とresizeで再計算。モーダルは対象外（手動ズームがあるため）
...
## その他の機能
- **結果表の画像出力**（`shareResultImage(g, kind)`/`buildResultCanvas`）: 対局履歴・その他成績の両カードに「画像」ボタン
  （ゲーム画面・結果詳細の計4箇所）。**画面の状態（フィット縮小の倍率など）に関係なく、Canvasに描き直した一枚絵PNG**を生成。
  DOMスクショ方式ではなくCanvas直描画（iOS SafariのforeignObject非互換を避ける＋常に崩れない絵にするため）。
  レイアウト: 上部ブランド帯（緑・三麻は青。グループ名/日付/試合数/表の種別）→表（列幅は文字の実測、
  長い名前はセル内で最大3行折り返し、チームカラー●・区切り線・ベスト色を再現）→右下majasco.jp。
  中身は対局履歴=スコアのみ+合計行／その他成績=転置表そのまま。
  出力はスマホ=共有シート（navigator.share・キャンセル時は何もしない）／非対応=PNGダウンロード。track('image_export')
- **CSVエクスポート**（`exportCsv(g)`/`buildCsv(g)`）: ゲーム画面と結果詳細の対局履歴カードに「CSV」ボタン
  ...
  列順は対局履歴と同じ（チームA→B…）。チップ/焼き鳥/チョンボ列は**設定有効または記録ありのときだけ**出す（旧データ互換）。
  不参加は空欄・マークは○。ファイル名「まじゃすこ_グループ名_YYYYMMDD.csv」（禁止文字は_に置換）。track('csv_export')

...
- **「あなたはどれ？」**（ゲーム画面の最上部`#askme-card`）: 自分を選んでいない・観戦でもない・「あとで」を押していないゲームで出す。
  割り勘アプリTricountの「開いた人に一度だけ聞く」方式。これまで自分に選んだ名前（`myKnownNameKeys`）が
  このゲームにいれば「あなたは『○○』ですか？ はい／ほかの名前／観戦だけ／あとで」、いなければ名前のチップを並べる。
  **全員が仮名（A・B…＝`isPlaceholderName`）のゲームでは聞かない**。「あとで」はこの端末でそのゲームには二度と聞かない
  （localStorage `mahjong-askme-skip-v1`・最大300件）。答えは`setTag`＝既存の参加/観戦タグと同じ（引き継ぎURLにも同期）
- **まとめて選ぶ**（過去の試合`#hist-organize`→`openOrganize`）: 自分未選択の参加卓（試合あり・仮名だけの卓は除く＝`untaggedGames`）を一覧にし、
  ...
  卓ごとのselectで個別に直せる（選ばない／各名前／観戦だけ）。保存で一括`setTag`
...
- 旧「過去の試合」画面（`view-history`）は**マイページ**: ログイン中の表示とログアウト／自分の成績シート／まとめて選ぶ／仲間ページ／過去の試合の一覧／ホーム画面に追加の案内。
  引き継ぎURLのカードと「この端末だけに保存」の注意書きは廃止（`renderMyListCard`は何もしない関数として残す）
...
- 「あなたはどれ？」はログイン中だけ出す（その場限りで遊ぶ人には聞かない）
...
## テスト
- /tests.html = index.htmlをiframeで読み実物関数を40ケース検証（うち16件が送信キューの applyOp、6件が対戦成績の h2hAdd・personKey）。**計算ロジック・保存処理の変更時はALL PASS確認必須**
...
- tools/account_test.py = ログイン・アカウント同期・仲間ページ（作成・追加・招待/参加・本人の区別・管理者・閲覧・LINE・ログインをまたぐ参加）・共有URLを**スタブのFirebase**で確認（本番DB・Googleに触らない）。

...
- `tests.html` — 計算ロジックの単体テスト（noindex）。index.htmlをiframeで読み込み、実物の
  rankGroups/calcScores/applyChombo/gameTotals/playedFlags/chipTotals/buildCsv を20ケース検証する。
...
- セクション順: ヒーロー → 1行圧縮（つかみ） → 3ステップ「必要なものはスマホのみ。」 → 比較表「手書きのスコア表は、もう卒業。」 →
  特徴3項目（700px以上で横並び.features-row）「電卓はもう、いらない。」 → クロージング「麻雀のスコア記録は、まじゃすこで。」（まじゃすこだけ緑）
  ※物語セクションと「過去の対局を振り返る」リンクは廃止済み
- 各セクション見出しはfont-size:25px/weight:900で統一
- ヘッダー: ロゴ52px・サブテキストなし・右上に「使い方」リンク（goUsage()で#howtoへ）
- 下部コンテンツ: #howto「1分でわかるまじゃすこ（majasco）の使い方」/ #basics スコア計算の基本 / #faq よくある質問（FAQPage JSON-LDと本文は一致させる）

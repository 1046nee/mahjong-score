...
- フック見出し（h1・全部緑#177083・太く大きく）:「麻雀のスコア記録、まだ紙とペンでやってますか？」
- ファーストビュー必須文言:「「まじゃすこ」は、表示点数を入力するだけでウマ・オカ込みのスコアを自動計算して、URLひとつで全員のスマホに共有できる無料のサービスです。」
- ヒーローにインラインSVGイラスト（電卓・くしゃくしゃの紙・ペンをゴミ箱へ／スマホ左上の「ま」アプリを指でタップ）
...
- セクション順: ヒーロー → 1行圧縮（つかみ） → 3ステップ「必要なものはスマホのみ。」 → 比較表「手書きのスコア表は、もう卒業。」 →
  特徴3項目「スマホ1台で、ここまでできる。」（モバイル=横スワイプカード .features-row、700px以上=3列グリッド） →
  クロージング「麻雀のスコア記録は、まじゃすこで。」（まじゃすこだけ緑）
  ※物語セクションと「過去の対局を振り返る」リンクは廃止済み
- 各セクション見出しはfont-size:25px/weight:900で統一
- ヘッダー: ロゴ52px・サブテキストなし・右上に「使い方」リンク（goUsage()で#howtoへ）
- 下部コンテンツ: #howto「1分でわかるまじゃすこ（majasco）の使い方」= Walica風の4ステップ
  （各ステップ: 中央見出し → 緑ブロック.howto-shotにスマホ画面のインラインSVG → 短い説明文）/ #basics スコア計算の基本 / #faq よくある質問（FAQPage JSON-LDと本文は一致させる）
...
### セットアップ画面
**1つの統合カード**（グループ名 → 対局形式トグル → 対戦形式トグル（チーム戦時は直下に編成エディタ） → メンバー名）→ ルール設定カード → オプションカード
...
- 対局履歴: **常に表形式**（1行目=名前、以降が試合。メンバーが多いときは表内で横スクロール。カード形式は2026.07.11に廃止）。
  行タップで修正・削除。ヘッダーの「拡大」ボタンで全画面モーダル（`openTableModal`・閲覧専用・#table-modal）に大きく表示。
  CSVエクスポートは廃止済み
...
- 個人成績のオレンジ強調は平均着順・ラス回避のみ（1着回数には付けない）

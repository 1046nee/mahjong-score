...
- **「収支」という語は使わない**（金銭のやりとりを直接指す語）。倍率を掛けた値は**「換算」**と呼ぶ:
  スコア換算 / チップ換算 / 換算合計。**表示単位の「bonus」も出さない**（数字だけ。行見出しに倍率を書く）
- **「スコア倍率」「チップ倍率」はLP・記事・FAQ・お知らせ・規約に出さない**。
  アプリ内でも設定はオプションカードの中だけ（既定は倍率「なし」）。
  公開ページで使う語は「スコア（pts）」「チップの記録」
- ただし**保存キーとCSSクラス・JS変数名は変えない**（`bonusEnabled` `chipRate` `startChips`、
  `.bonus-section` `.score-bonus` など）。データ互換性が優先で、これらは画面に出ない
- 提供価値は**「スコア記録」**と呼ぶ（行為の説明として「スコアを自動計算」はOK）
- 倍率を掛けた値の計算は スコア換算=pts×スコア倍率／チップ換算=チップ差×チップ倍率／換算合計=和。
  チップ差=終了時チップ−開始時チップ（持ちチップ）。**画面に単位は出さない**（行見出しに倍率を書く）

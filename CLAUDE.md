# まじゃすこ / majasco — 開発ガイド（コア）

麻雀のスコア記録・共有ができる無料Webアプリ。ビルド不要の静的サイト（Vanilla JS + Firebase Realtime Database + Firebase Hosting）。

- 本番: https://majasco.jp ／ リポジトリ: https://github.com/1046nee/mahjong-score（ブランチ: main）
- mainへpush → GitHub Actionsが自動デプロイ（追加作業不要）
- SNS: X公式 @majasco_jp ／ Instagram公式 @majasco_jp ／ Threads @munii_dev（むにぃ個人・収益発信用。詳細は docs/sns-ops.md）
- 画像素材: privateリポジトリ https://github.com/1046nee/majasco-assets（`まじゃすこ素材/` にnested cloneで配置。公開リポジトリには入れない）
- 運用ハブ（Notion・ユーザー管理）: https://app.notion.com/p/39afc00e6ad081f2b1ccc874b1818eee
  （DB: 資産/アカウント一覧・公開ページ一覧・運用タスク・**SNS投稿カレンダー**（投稿文は各行のページ内）＋用語辞典（初心者向け解説56語）。
  記事公開時は公開ページ一覧にも1行追加。仕様の正はあくまでdocs/。NotionはMCPツール notion-* で読み書きできる）

## このファイルの使い方（最初に読む）
ここには**全作業で守るルールだけ**を置く。仕様の詳細は docs/ に分割してある。
**タスクを受けたら、下の表で該当するdocだけを読んでから作業に入ること。全部読む必要はない。**

| タスクの種類 | 読むdoc |
|---|---|
| アプリの機能・画面・計算（index.html） | docs/app-spec.md |
| LP（トップページの見た目・文言・イラスト） | docs/lp-spec.md |
| ブログ記事の追加・修正 | docs/blog-spec.md |
| サイト共通（新ページ・SEO・広告・計測・固定ページ・デプロイ構成） | docs/site-spec.md |
| SNS（X・Instagram・Threadsの投稿文・画像・運用） | docs/sns-ops.md（投稿予定と文面はNotionのSNS投稿カレンダー） |
| 画像生成（OGP・SNS画像・イラスト） | docs/image-tools.md |
| 別PCのセットアップ・環境構築 | docs/setup-new-pc.md |

複数領域にまたがるときは該当するdocを複数読む（例: 新機能→app-spec＋lp-spec、リリース告知→blog-spec＋sns-ops）。

## 絶対ルール（全タスク共通）
1. **作業前に必ず `git pull origin main`**（ユーザーがGitHub Web UIで直接編集することがある。巻き戻し厳禁）
2. 変更後は**ブラウザ実機で検証してから完了報告**（mcp__Claude_Browser__*。スクリーンショットはタイムアウトしやすいのでDOM評価を主体に。検証で作ったFirebaseセッションは `db.ref('sessions/ID').remove()` で削除）
3. **計算ロジックを触ったら /tests.html がALL PASSであることを確認**してからコミット
3b. **主要動線（LPのCTA・グループ作成・共有URL・点数入力）を触ったら `python tools/smoke_test.py` を通す**。
   これはCIがpushごとに実行し、落ちるとデプロイが止まる。動線を変えたらテスト側も同時に直すこと
3c. **保存処理（Firebaseへの書き込み）を触ったら `python tools/sync_test.py` を通す**（本番DBで2台の端末を再現して検証。
   検証用ゲームは自動で削除）。保存は送信キュー `enqueueOp` 経由のみ。**`sessions/{id}` 直下に新しいキーを足さない**
   （本番ルールで全部弾かれる。スタブのテストでは気づけない。詳細は docs/app-spec.md「送信キュー」）
3d. **ログイン・引き継ぎ・仲間ページ・共有URLを触ったら `python tools/account_test.py` を通す**（スタブで検証。本番DBに触らない）。
   仲間ページのルールを変えたら、コンソールに貼ってから `python tools/circle_rules_test.py`（本番DB・匿名ログインが必要）も通す。
   見るだけのリンク（views / sessionViews）のルールを変えたら `python tools/view_rules_test.py`（本番DB・ログイン不要）。
   試合の記録の確定（sessions の lockAt・endedAt）のルールを変えたら `python tools/lock_rules_test.py`（本番DB・ログイン不要）
4. 変更ごとに `git add → commit（日本語で内容と理由）→ push origin main` まで行う
5. **データ互換性が最優先**: 旧データはキー無し=無効扱いで計算に影響させない。保存キー名は変えない（settings.rate等bonus系のまま）
6. 大きめの置換はscratchpadにPythonスクリプトを書いて実行する（アンカー文字列をassertで検証してから置換。ヒアドキュメント直書きはエスケープ事故のもと）
7. **トラブルを修正したら、該当docの「## 落とし穴」に1行追記する**（同じ失敗を繰り返さないための記憶。新しい学びも同様）
8. 仕様を変えたら該当docの記述も更新する（docには「今の正」だけを書く。経緯はgit log）。このCLAUDE.mdは薄く保つ

## 用語ルール（AdSense対策。UI・記事・SNSすべての文言で厳守）
- 「円」「金額」「精算」「賭け」「レート」「係数」など現実の金銭を連想させる語は**禁止**
- **「pts→bonus」の換算例は書かない**（例：「×100なら +123.0pts → +12,300bonus」）。
  換算式を見せると、単位名を変えただけの金銭換算機として読まれる。2026-08-26に全ページから削除した
- **「収支」という語は使わない**（金銭のやりとりを直接指す語）
- **スコア倍率・チップ倍率・換算は2026-09-25にアプリから機能ごと廃止した。復活させない**。
  「倍率」「換算」をUI・記事・FAQ・お知らせ・規約に出さない。公開ページで使う語は「スコア（pts）」「チップの記録」
- ただし**保存キーとCSSクラス・JS変数名は変えない**（`bonusEnabled` `rate` `chipRate` `startChips`、
  `.score-bonus` など）。データ互換性が優先で、これらは画面に出ない（旧データの rate / chipRate は読むだけ）
- 提供価値は**「スコア記録」**と呼ぶ（行為の説明として「スコアを自動計算」はOK）
- チップ差=終了時チップ−開始時チップ（持ちチップ）。記録するのは枚数の増減だけ
- **SNS画像・OGPにも同じルールが効く**（tools/の生成スクリプト。詳細は docs/image-tools.md）
- **「三麻」「四麻」**表記（初出やSEOタイトルは「三麻（3人麻雀）」併記可）。アプリ内の履歴は「過去の試合」
- 符・翻・役の**「点数計算」はこのアプリの対象外**。アプリが解決するのは対局後の「スコア計算/集計」。
  見出し・説明での混同禁止（詳しい例: docs/blog-spec.md）

## デザインシステム
- 緑 `#177083`（メイン）／オレンジ `#E75620`（**CTAボタン限定**）／薄緑 `#E4F3EC`／薄青 `#EAF0FB`／水色背景 `#DBF4F9`
- 三麻（サンマ）モードは緑→青 `#4261AA` に置換（`.sanma`クラス）
- 着順の色: 1着 `#D4A017`・2着 `#6E8CA8`・3着 `#B4C0BD`・ラス `#C92F42`（三麻の3着＝ラス）。「位」は金銀銅のメダル。詳細は docs/app-spec.md「色とアイコン」
- **絵文字は使わない**。アイコンはスプライト＋`ico(名前, サイズ)`（LPの特徴カードと同じ描き方）。オレンジは小・中のアイコンに使わない
- Walica風のシンプルで丸いフラットデザインを参考に（模倣はしない）。LPコピーは行動経済学（損失回避・アンカリング等）を意識

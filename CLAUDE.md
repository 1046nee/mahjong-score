...
- **プレースホルダーは例の値をそのまま置く**（グループ名=まじゃすこ学園サッカー部、メンバー名=山田、数値欄=150 等。「（例）」の接頭辞は付けない。
...
- **全画面ページ形式**（2026.07.11に下からのスライド式ボトムシートを廃止。iOSでtransform＋fixed＋inputのタップ不良が起きるため。
  .sheetはfixed inset:0・max-width:560px中央・transformなし・タイトル右に✕閉じるボタン）
...
- 対局履歴: **常に表形式**（1行目=名前、以降が試合。メンバーが多いときは表内で横スクロール）。
  ...
  **その他成績にも「もっと見る」**（`openStatsModal`・同じモーダルとズームを共用）。CSVエクスポートは廃止済み

...
- ヒーローにインラインSVGイラスト＝**過去と現在の対比**: 左（電卓・紙・鉛筆をゴミ箱へ）は opacity 0.55 で薄く・鉛筆も電卓と同じくすんだトーン
  （鉛筆は**消しゴム・金具なし**の軸＋芯のみ。ゴミ箱の真上に配置し、電卓の背面レイヤーに重ねる）、
  点線の乗り換え矢印（進行方向に沿った三角の矢じり）、右のスマホ（スコア収支画面・iPhone風）は淡い光の円＋**4方向に尖った✨型スパークル**で明るく。
  ＋印のキラキラ・指のイラストは廃止
- CTAボタンは影なし・横長ピル型（width:100%/max-width:380px）。**CTA下はバッジ3つ**（.close-badges: 登録不要/完全無料/スコアを共有。
  クロージングと同じ表記で統一。旧・緑太字2行の注記は廃止）
...
- セクション順: ヒーロー → 3ステップ「必要なものはスマホのみ。」 → 比較表「手書きのスコア表は、もう卒業。」 →
  ...
  ※物語セクション・1行圧縮（つかみ）・「過去の対局を振り返る」リンク・「もっと詳しく」カードは廃止済み
- **各セクションは「タイトル→表示→締め」の3段構成**（.start-noteの締め文言: 3ステップ=「たった10秒で、次の対局をスタート。」／
  比較表=「めんどうな計算は、ぜんぶ まじゃすこへ。」（ぜんぶの後は半角スペース）／特徴=「そしてこの全部が、無料。」）
- クロージングは**ミニイラスト（スマホ＋✨のSVG .close-illust）＋ひとこと2行＋バッジ3つ（.close-badges: 登録不要/完全無料/スコアを共有）＋CTA**
- 特徴カードは**通常のカルーセル**（2026.07.11に無限ループは廃止。`initFeaturesCarousel`）。
  カード幅はモバイル62%（**常に次のカードが半分ほど見える**）・snap-align:start・700px以上は300px固定＋左右矢印 .features-nav。
  **UX演出**: 両端28pxの白フェード＋下に現在位置ドット `.features-dots`（画面中央に最も近いカードを点灯。rAFは使わず直接更新）
- 特徴は**8項目**（タイトルは1行に収まる8〜10字・16px・本文は2文で厚めに）: スコアを一瞬で計算／全員のスマホに共有／推移グラフも自動／
...
- 文字サイズは視認性優先で**解説系の本文は15pxに統一**（hero-desc/使い方の説明文/faq.html/score-basics.htmlの本文とも15px。feature pのみ14.5px。行間は1.8前後）
- 各セクション見出しはfont-size:22px/weight:900で統一（使い方の見出しと同サイズ。2026.07.11に25px→22px）
...
- 下部コンテンツ: #howto「1分でわかる「まじゃすこ」の使い方」= Walica風の4ステップ。
  **カード（info-card）にはせずシンプルな置き方**。見出しh2=22px/900・各ステップh3=20px/900・説明文15px。
  イントロは「会員登録もインストールも不要。」で改行
  （各ステップ: 中央見出し → 緑ブロック.howto-shotにスマホ画面のインラインSVG → 短い説明文。
  スマホフレームはiPhone風: viewBox 220×400・本体比率約1:1.94・ベゼル8px均一・上部にダイナミックアイランド）
  → lp_bottom広告枠。※スコア計算の基本・よくある質問はLPから独立ページへ移行済み（リンクはフッターから）
...
- **モバイルの横はみ出し対策（2026.07.11）**: `.post-layout > * { min-width: 0 }`が必須（グリッドアイテムは幅広コンテンツで
  本文列ごと広がり右が切れるため）。記事内の表はblog-site.jsが`.table-scroll`に包み、**4列以上はmin-width:520pxで横スクロール**
  （潰して詰め込まない）。body { overflow-x: hidden } の保険つき
- **記事のイラストは3点構成**: 冒頭（.post-illust 340×150）＋中盤（.post-illust.small 340×84）＋締め（トロフィーの共通SVG・
  記事末尾のarticle_bottom広告枠の直前）。絵文字は使わずインラインSVG（サイト共通のフラットなトーン）。
  **イラスト内のスマホに「まじゃすこ」等の文字は入れない**（サイズが合わないため。2026.07.11に全記事から削除）
- **記事のCTAボタン（.cta-link/.app-btn）はLPのCTAと同じ形**（影なし・横長ピル radius50px・max-width380px中央・16px/800。
  blog.cssの!important上書きで統一。各記事のインラインCSSは触らなくてよい）
- 記事の左右余白は .post-layout padding 8px **26px** 44px（右寄り感の解消のため20px→26px）
...
- **プレースホルダーは例の値をそのまま置く**（グループ名=まじゃすこ学園サッカー部、メンバー名=山田先輩、数値欄=150 等。「（例）」の接頭辞は付けない。
  グループ名に日付を入れる前提はやめた。**作成日はゲーム画面・結果詳細のタイトル下に自動表示**（#game-date / #detail-date、g.createdAt）
- 開始条件: メンバー数 ≥ numPlayers。不足時は「ゲームを開始するにはあと[X]名必要です」＋開始ボタン無効
- チップ倍率はスコア倍率と同じセレクト＋カスタム形式（デフォルト×500）。開始時チップ数の初期値は20
- **オプション名（2026.07.11改訂）**: 「チップあり🍪」「焼き鳥あり🐔」「チョンボあり💩」（旧: チョンボ罰符あり）。
  各説明文は「本文。<br>※注意書き。」の2行形式（例: 焼き鳥=「1試合で一度もアガれなかった人に🐔マークがつきます。<br>※マークのみでスコアには影響しません。」）。
  倍率の例示は「例：スコア倍率が×100の場合、+123.0pts → +12,300bonus」「例：チップ倍率が×500の場合、+5枚 → +2,500bonus」の形式
- **「持ちチップ（枚）」**（旧: ゲーム開始時チップ数。保存キーは`settings.startChips`のまま）。配置は**持ちチップ（左）・チップ倍率（右）**
- 「登録名の管理」ボタンは**「メンバー名（最大40人）」ラベルの右**（常時表示。「入力欄をタップすると…」の案内文は廃止）
- **三麻を選ぶとセットアップ画面全体がサンマカラー（青#4261AA）**（view-setupに.sanmaクラス。トグルの選択色・カード枠が青になる）
...
- チーム編成: 名前をタップで次のチームへ移動（ガイドはタップのみ表記。D&Dも動くが案内しない）・チーム追加/削除（最低2）・🎲ランダム編成・チーム名編集可。
  **各チームボックスにチームカラー**（左5pxボーダー＋名前入力の前に●。TEAM_COLORS順。セットアップと設定変更モーダルで共通）
- グループ名の初期値は**空**（プレースホルダーのみ。未入力で開始したときだけstartGameが日付名にフォールバック）
- 登録名管理モーダル（2026.07.11改修）: 通常モードは各名前に[ゲームに追加]／追加済みなら**[取り消し]**（ゲームから外すだけ。登録名は残る）。
  右上の**[編集]**で編集モード（↑↓で並び替え・[削除]）。[完了]で通常モードへ。行に削除ボタンは常設しない

### スコア入力シート
- **「設定の編集」と同じ中央モーダル形式**（2026.07.11確定。スライド式→全画面ページを経て最終形。
  .sheet=暗背景オーバーレイ・.sheet-box=白パネル（max-width:430px・max-height:88vh・パネル内スクロール）・transformなし）
- **タイトル右に小さい「キャンセル/完了」＋最下部に「入力完了/キャンセル」の両方**（#confirm-btn-topは下の入力完了と同じ活性状態に同期。
  キャンセルは常時表示）。**タイトルに試合番号**（「第N試合の点数入力」「第N試合を修正」）。背景タップでも閉じる
- メンバー数>numPlayersのグループのみ「今回の対局メンバー」選択チップを表示（デフォルトは直前試合のメンバー）
- タイトルは収支ONで「点数 / チップ入力」。行は「名前｜チップ（左）｜点数（右）｜🐔｜💩」の1行レイアウト（🐔=焼き鳥ON時・💩=チョンボON時のみ）
- **入力欄はコンパクト固定幅**（チップ68px=3桁・点数104px=4桁+「00」・名前56px・行gap8px＝モーダル幅430pxに🐔💩込みで収まる。
  **チップと点数は隣接**＝margin autoで離さない）。
  **整数のみ入力可**（`blockNonInt`が.,eE+をブロック。マイナスは箱下用に許可）
- 点数合計とチップ合計のチェック表示は**同サイズ（13px/padding8px）**。「残り1人を自動入力」と「入力完了」ボタンも**同サイズ（15px/padding13px）**
- チーム戦では「今回の対局メンバー」チップも`memberDisplayOrder`でチームA→B…順
...
- **全カードの右上に開閉トグル**（`.card-collapse`丸ボタン ▾/▸。状態は`collapsedCards`（key: team/score/rounds/rank/chart/settings）、
  `toggleCard(key)`がアクティブなビューを再描画。グループや詳細を開き直すと全カード表示にリセット。
  たたんだカードは「もっと見る」等のアクションボタンも隠す）
- **0試合の間は順位バッジを出さない**（総合成績カードのバッジ部分は灰色の「—」。1試合目の入力後から順位・メダルを表示）
...
- 対局履歴: **常に表形式**（1行目=名前、以降が試合。メンバーが多いときは表内で横スクロール）。
  行タップで修正・削除。ヘッダーの**「もっと見る」**ボタン（旧「拡大」）で全画面モーダル（`openTableModal`・閲覧専用・#table-modal）。
  モーダルは列幅コンパクト（min-width 46+N×56px）＋「−/＋」ズームボタン（CSS zoom・40〜200%）で全列を一画面に収められる。
  **その他成績・スコア推移にも「もっと見る」**（`openStatsModal`/`openChartModal`・同じモーダルとズームを共用）。
  モーダルのヘッダーは**タイトル行（タイトル＋閉じる）とサブ行（ズーム±・注意書き）の2段構成**＝長いグループ名でも切れない（タイトルは折返し可）。
  注意書き（.table-modal-note）「拡大・縮小の倍率によっては文字やレイアウトが崩れる場合があります」。CSVエクスポートは廃止済み
- **その他成績**（2026.07.11に「個人成績」から改名。`rankCountHtml`・転置）: 行=メンバー、
  列=**スコア/素点/順位点**（+青#1a237e・−赤#c62828・0黒）/1着..n着/試合数/平均着順/**連対率**/ラス回避/**平均点数・最高点数・最低点数**。
  素点＝(持ち点−返し点)÷1000の合計、順位点＝スコア−素点（ウマ・オカ・チョンボ罰符ぶん）、連対率＝(1着+2着)÷試合数。名前列はsticky固定。
  ベスト（平均着順最小・連対率最大・ラス回避最大。同値は該当者全員）は**文字色オレンジ#E75620のみ**で強調（背景・枠線は付けない。1着回数には付けない）。
  チーム戦はチームごとにグループ化（見出し行は10.5pxの控えめ表示・色はチームカラーの●のみで文字色は変えない）
- **チーム識別はカラー**（2026.07.11に記号◯△◻✕を廃止）: `TEAM_COLORS`＝チームAから順に
  ...
  メンバー名の前に`.team-dot`（●）を付ける（`teamDotHtml`）。**名前の文字色は変えない**（色はドットだけ）。
  ドットを付ける場所: 総合成績・対局履歴ヘッダー・内訳・その他成績・点数入力シート（対局メンバー選択チップ＋入力行の名前）。
  **チーム戦では対局履歴・その他成績・内訳の表1行目を灰色背景（TEAM_HEAD_BG=#5f6e6b・白文字＋色ドット）**にする（黒は2026.07.11にトーン不一致で廃止）。
  対局履歴の列と合計収支（ミニカード＋内訳の列）は`memberDisplayOrder`で**チームA→B…の順**に並べる（個人戦の合計収支は値の大きい順のまま）
...
- タイトル横のボタンは**「設定の編集」**（旧「変更」。`.title-edit-btn`・通常緑・三麻モードでは青#4261AA）
...
- 過去の試合の結果概要は**トップ3だけの1行表示**（🥇🥈🥉＋名前＋スコア＋「ほかN人」。表は出さない。全員分はタップして結果詳細で見る）
- **履歴ログ（対局履歴の「履歴」モーダル）**: 上部に「すべて/入力/修正/削除」のフィルタタブ（logFilter・renderLogModal）。
  タブの色は履歴タグ（.log-tag）と同じ配色＝入力:緑#177083／修正:オレンジ#E75620／削除:赤#c62828（非選択時は文字色、選択時は背景色）。
  点数は**「名前 点数」のペア**で表示＝対戦相手が分かる（新規ログはe.namesに参加者名を保存。旧ログは現在の試合番号から補完）

### 設定変更モーダル（ゲーム中の「変更」）
- 対局形式（3人/4人）は変更不可（モーダルに明記）。ルール変更は過去試合を全再計算（⚠警告を表示）
- メンバー追加可。削除は「対局済み」「最低N人」の場合ラベル表示で不可。削除時はrounds[].membersとチームのindexを振り直す
- モーダルの**ヘッダー右に小さい「キャンセル/保存」**（#form-modal-actions。フォームが長く最下部のボタンまで遠いため。
  closeFormModalがクリアするので、他のモーダルには残らない）

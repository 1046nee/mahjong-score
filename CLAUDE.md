...
### フェーズ2: ボーナス計算ロジック 【完了 2026.07.10】
1. ゲーム設定（セットアップ+設定変更モーダル）に「ボーナスオプション」を追加:
   「ボーナス計算を有効にする」チェック（ONでチップ倍率欄を表示）＋「焼き鳥🐔を記録する」チェック＋注釈（例: 100.0pts × スコア倍率10倍 → 1000スコアボーナス）
2. スコア入力シート: ボーナス計算ONのグループのみメンバーごとの「ボーナス枚数」欄（枚・負数可・合計検証なし）と🐔トグルを表示。
   値はメンバーindexキーで保持（sheetChips/sheetYaki）し、対局メンバー切替でも消えない
3. ゲーム画面・結果詳細に「ボーナス集計」カード: 総合ボーナス行のみ常時表示、内訳（スコア合計/スコアボーナス/ボーナス枚数/チップボーナス/焼き鳥回数）は「内訳を表示」タップで展開（`.bonus-table-wrap.bonus-collapsed`）。計算は `bonusTableHtml(g)` に一元化
4. 保存キー: `settings.bonusEnabled`（bool）/ `settings.chipRate`（数値・未設定は1倍扱い）/ `settings.yakitori`（bool）、
   試合ごとに `round.chips`（枚数配列・scoresと同じslot順）/ `round.yakitori`（0/1配列）。**旧データはキー無し=無効/0枚として一切影響なし**
5. 対局履歴の表は記録がある試合のみセル内に「+3枚」「🐔」を小さく表示

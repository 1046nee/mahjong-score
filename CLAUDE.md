...
3. ゲーム画面・結果詳細に「収支集計」カード: 合計収支行のみ常時表示、内訳（スコア合計/スコア収支/チップ差/チップ数=開始時+差/チップ収支/焼き鳥回数）は「内訳を表示」タップで展開（`.bonus-table-wrap.bonus-collapsed`）。計算は `bonusTableHtml(g)` に一元化

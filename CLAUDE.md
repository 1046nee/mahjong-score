...
### スコア入力シート
- **全画面ページ形式**（2026.07.11に下からのスライド式ボトムシートを廃止。iOSでtransform＋fixed＋inputのタップ不良が起きるため。
  .sheetはfixed inset:0・max-width:560px中央・transformなし）
- **見た目**: ブランドカラーのstickyヘッダー（緑#177083・三麻は青#4261AA・白タイトル＋✕）＋薄緑背景#f6fbf9＋
  白カード（.sheet-card: 対局メンバー選択／入力欄）。**タイトルに試合番号**（「第N試合の点数入力」「第N試合を修正」）
- メンバー数>numPlayersのグループのみ「今回の対局メンバー」選択チップを表示（デフォルトは直前試合のメンバー）
- タイトルは収支ONで「点数 / チップ入力」。行は「名前｜チップ（左）｜点数（右）｜🐔｜💩」の1行レイアウト（🐔=焼き鳥ON時・💩=チョンボON時のみ）
- **入力欄はコンパクト固定幅**（チップ72px=3桁・点数108px=4桁+「00」。**チップと点数は隣接**＝margin autoで離さない）。
...
- 対局履歴: **常に表形式**（1行目=名前、以降が試合。メンバーが多いときは表内で横スクロール）。
  ...
  **その他成績・スコア推移にも「もっと見る」**（`openStatsModal`/`openChartModal`・同じモーダルとズームを共用）。
  モーダルのヘッダー下に「拡大・縮小の倍率によっては文字やレイアウトが崩れる場合があります」の注意書き（.table-modal-note）。CSVエクスポートは廃止済み

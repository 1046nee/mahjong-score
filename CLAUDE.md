...
- **各セクションは「タイトル→表示→締め」の3段構成**（.start-noteの締め文言: 3ステップ=「たった10秒で、次の対局をスタート。」／
  比較表=「めんどうな計算は、ぜんぶまじゃすこへ。」／特徴=「そしてこの全部が、無料。」）
- クロージングは**ミニイラスト（スマホ＋✨のSVG .close-illust）＋ひとこと2行＋バッジ3つ（.close-badges: 登録不要/完全無料/10秒でスタート）＋CTA**
...
- 下部コンテンツ: #howto「1分でわかる「まじゃすこ」の使い方」= Walica風の4ステップ。
  **カード（info-card）にはせずシンプルな置き方**。見出しh2=25px/900・各ステップh3=20px/900（2026.07.11強化）
...
- **プレースホルダーは「（例）〜」形式で統一**（グループ名=（例）まじゃすこ学園サッカー部、メンバー名=（例）山田、数値欄=（例）150 等。
